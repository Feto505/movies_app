...
import 'package:movie/api/network/remot/firebase_finctions.dart';
...
class AddMovie extends StatefulWidget {
  ...
  AddMovie({super.key, required this.result});
...
class _AddMovieState extends State<AddMovie> {
  ...
  getMovieFromWatchList() {
    FirebaseFunctions.searchMovieById(widget.result.id.toString())
        .then((value) {
      setState(() {
        isAdded = value;
      });
  ...
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () {
        if (isAdded == false) {
          final MovieWatchListModel movie = MovieWatchListModel(
